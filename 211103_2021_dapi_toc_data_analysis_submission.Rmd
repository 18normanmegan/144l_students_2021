...
dapi_data <- read_excel("Input_Data/week4/144L_2021_BactAbund.xlsx", sheet = "DAPI_Data")
glimpse(dapi_data)

toc_data <- read_excel("Input_Data/week4/144L_2021_BactAbund.xlsx", sheet = "TOC_Data")
glimpse(toc_data)
...
A1: The differences between the treatments include the highest cell abundance level in the GNP treatment and the Kelp+NP treatment, followed by the Kelp exudate treatment, and the lowest cell abundance in the control treatment.
...
A2: The cell abundance data has a different trend from the biovolume data. While the cell abundance data has a trend that goes mostly upward, the biovolume data increases in the middle of the experiment and then decreases again. Across treatments, the GNP and Kelp+NP treatments were approximately the highest in biovolume, which is the same as the cell abundance data. Again, the kelp exudate was below these, and the control was approximately the lowest biovolume treatment, which is similar to the cell abundance data.
...
A3: Specific growth rates must be calculated using the data points in the exponential phase of growth. One limitation of using the DAPI data is that there are only three time points for each treatment (0, 4, and 8) and exponential growth only occurs between the time points 0 and 4. So, we will only get to see how they changed between two points in time.
...
write.csv(bactcarbon, "Output_Data/week6/211101_dapi_data_calculations.csv")
...
saveRDS(trt_ba, "Output_Data/week6/144L_trt_ba_plot.rds")
```
...
saveRDS(trt_ba_biovol, "Output_Data/week6/144L_trt_ba_biovol_plot.rds")
```

Q4: How do the bacterial carbon values change when we account for changes in cell biovolume as the experiment progresses? 

A4: Biovolume-derived bacterial carbon results in an increase in the values for the GNP treatment and Kelp+NP, and a slight increase in the values for the Kelp exudate and the control. In the graph of biovolume-derived bacterial carbon, this is evident in the separation between the top two lines and the bottom two. This separation is not as evident in the plain bacterial carbon graph.
...
A5: The patterns in these data (∆Cells, specific growth rate (u), and doubling time) all line up with the previous observations of cell abundance which suggested that the GNP treatment had the most successful growth, followed by the Kelp+NP, Kelp exudate, and control. They also followed the oxygen drawdown data, which suggested the treatments followed this pattern in terms of decreasing cell growth and abundance.

# Save Data 

```{r}
saveRDS(bactcarbon, "Output_Data/week6/144L_Exp_Processed_BactAbund.rds")
```
...
A6: a) Our target concentration of carbon was 100 µmol C L"^-1.
    b) No, the Control, Kelp exudate, and Kelp+NP treatments were all much lower than the GNP treatment.
    c) Our ability to assess the effect of the type of DOC is not as strong, since the factor being assessed could very well be the amount of the DOC rather than the type.
    d) Our TOC over time decreases, which aligns with the data about our cell abundances increasing, since microbes will decrease more TOC by converting it to inorganic carbon as their numbers grow.
...
A7: Incorporating biovolume for DOC (a+b) increases the DOC measurements for most of the  treatments. Incorporating biovolume for DOC fraction (c+d) causes an increase in the measurements for the GNP and Kelp+NP treatments. Incorporating biovolume for bacterial growth efficiency (e+f) causes the measurement for the control treatment to decrease while the rest of the measurements for the other treatments increase. 
