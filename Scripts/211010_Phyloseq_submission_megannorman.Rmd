...
count.tab <- read_rds("Input_Data/week7/seqtab-nochimtaxa.rds") #table of counts for each sequence in each sample
tax.tab <- read_rds("Input_Data/week7/taxa.rds") #table that matches ASV to sequence
...
metadata_OG <- read_excel("Input_Data/week7/ACIDD_Exp_BactAbund.xlsx", sheet = "Metadata")
glimpse(metadata_OG)
metadata <- metadata_OG %>%
  mutate(Datetime = ymd_hm(Datetime))
glimpse(metadata)
metadata_doc <- read_rds("Input_Data/week7/ACIDD_Exp_Processed_DOC_BGE.rds")
...
A1: The code chunk above created a subset of the sequence data in the taxa table that has any mitochondria or chloroplast sequences removed. We did this because we want to limit our sequences to bacterial ones.
...
A3: No, subsampling will not have a large effect on the beta diversity patterns because there is only a small difference between the minimum (6047) and the mean (7695). 
...
A4: The datasets used were the not-subsampled and subsampled datasets. The plots look very similar, so we will go forward with our subsampled (or standardized) dataset.
...
A5: The communities in San Diego diverged more over time than the Santa Barbara communities. The San Diego control and ash leachate communities were more dissimilar by the last timepoint than the Santa Barbara ones, which did change over time, but the similarity between the control and ash leachate did not change drastically.
...
A6: The richness for the control was higher than that of the ash leachate at the beginning of the experiment, which could be due to error. Over the time points, the control treatment decreased in richness and stayed around the same diversity. The ash leachate decreased in both richness and diversity over the time points.
...
A7: In the ash leachate treatment, the relative abundance of Campylobacteriales, Rhodobacteriales, Oceanospirillales, and Alteromonadales uniquely increased. None uniquely decreased in the ash leachate.
...
```{r}
saveRDS(sweet.tab, "Output_Data/week7/Custom_ASV_Table.rds")
saveRDS(sub_ps, "Output_Data/week7/phyloseq_obj.rds")
saveRDS(ps_min, "Output_Data/week7/subsampled_phyloseq_obj.rds")
saveRDS(alphadiv, "Output_Data/week7/alphadiv.rds")
